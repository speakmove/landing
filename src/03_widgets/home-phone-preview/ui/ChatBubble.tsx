import { BubbleTail, DoubleCheckIcon, PlayIcon, WaveformBars } from '@/shared/ui';
import { cn } from '@/shared/model/libs/cn';
import type { TPhoneMessage } from '../model/types';
import { buildWaveformBars } from '../model/waveform';

type TProps = {
  message: TPhoneMessage;
  index: number;
};

/**
 * Single Telegram-style voice-message bubble.
 * Renders play button + waveform + duration row, optional translation caption,
 * and a bottom-right timestamp with double-check for outgoing messages.
 * BubbleTail is the only absolute-positioned element (it sits outside the bubble edge).
 */
export const ChatBubble = ({ message, index }: TProps) => {
  const isMe = message.from === 'me';
  const seed = (index + 1) * 7;
  const bars = buildWaveformBars(seed);
  const duration = isMe ? '0:08' : '0:11';
  const timestamp = '11:00 AM';
  const accentText = isMe ? 'text-tg-bubble-me-accent' : 'text-tg-bubble-bot-accent';
  const accentDot = isMe ? 'bg-tg-bubble-me-accent' : 'bg-tg-bubble-bot-accent';
  const barFill = isMe ? 'fill-tg-bubble-me-accent' : 'fill-tg-bubble-bot-accent';

  return (
    <div className={isMe ? 'flex justify-end' : 'flex justify-start'}>
      <div
        className={cn(
          'tg-bubble-shadow relative flex max-w-[88%] flex-col gap-1.5 rounded-card bg-white px-3 py-2',
          isMe ? 'rounded-br-[3px] bg-tg-bubble-me' : 'rounded-bl-[3px]',
        )}
      >
        <BubbleTail
          side={isMe ? 'right' : 'left'}
          fill={isMe ? 'var(--color-tg-bubble-me)' : 'white'}
          className={isMe ? 'absolute -right-2 bottom-0' : 'absolute -left-2 bottom-0'}
        />

        {/* Voice row: play + waveform + duration */}
        <div className="flex items-center gap-3">
          <span
            className={cn(
              'grid h-11 w-11 flex-none place-items-center rounded-full text-white',
              isMe ? 'tg-play-me' : 'tg-play-bot',
            )}
          >
            <PlayIcon size={30} />
          </span>

          <div className="flex flex-col items-start gap-1">
            <WaveformBars bars={bars} className={cn('block', barFill)} />
            <span
              className={cn(
                'inline-flex items-center gap-1.5 text-xs font-medium leading-none',
                accentText,
              )}
            >
              {duration}
              <span className={cn('block h-1 w-1 rounded-full', accentDot)} />
            </span>
          </div>
        </div>

        {/* Optional translation as voice transcript */}
        {message.translation ? (
          <p
            className={cn(
              'm-0 max-w-65 text-xs leading-snug',
              isMe ? 'text-tg-bubble-me-accent' : 'text-tg-bubble-bot-muted',
            )}
          >
            {message.translation}
          </p>
        ) : null}

        {/* Timestamp + delivered checks — bottom-right via self-end in column flow */}
        <span
          className={cn(
            'inline-flex items-center gap-0.5 self-end whitespace-nowrap text-[0.6875rem] leading-none',
            isMe ? 'text-tg-bubble-me-accent' : 'text-tg-timestamp-muted',
          )}
        >
          {timestamp}
          <DoubleCheckIcon size={14} className="text-tg-bubble-me-accent" />
        </span>
      </div>
    </div>
  );
};
